...
if (!$is_admin && !$canManageChat && !$isRequester && !$isCurrentAssignee && !$hasSentInConversation) {
    http_response_code(403);
...
$storedAttachment = trim((string) ($messageRow['attachment_stored_name'] ?? ''));
if ($storedAttachment !== '') {
    $path = ticket_chat_upload_dir() . DIRECTORY_SEPARATOR . $storedAttachment;
    if (is_file($path)) {
        @unlink($path);
    }
}
